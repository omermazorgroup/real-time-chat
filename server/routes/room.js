 ...
 */
router.post('/update/name', [passport.authenticate('jwt', { session: false }), checkCreateRoomFields], async (req, res) => {
    let errors = [];
    const room = await Room.findOne({ name: req.body.room_name }).exec();

    if (room && room.name === req.body.room_name  && room.name !== req.body.current_room_name) {
            errors.push({ param: 'room_taken', msg: 'Roomname already taken' });
            return res.json({ errors: createErrorObject(errors) });
        }
    else {
        ...
        const room = await Room.findOneAndUpdate(
            { name: req.body.current_room_name },
            { name: req.body.room_name },
